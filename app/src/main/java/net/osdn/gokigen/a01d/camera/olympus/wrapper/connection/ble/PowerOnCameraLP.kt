...
class PowerOnCameraLP(private val context: Activity, private val camera: OLYCamera) : ICameraPowerOn
{
    ...
    private fun setupCameraList()
    {
        ...
        for (index in 1..IOlyCameraEntryList.MAX_STORE_PROPERTIES)
        {
            val idHeader = String.format(Locale.ENGLISH, "%03d", index)
            val prefDate = preferences.getString(idHeader + IOlyCameraEntryList.DATE_KEY, "") ?: ""
            if (prefDate.isEmpty())
            {
                // 登録が途中までだったとき
                break
            }
            val btName = preferences.getString(idHeader + IOlyCameraEntryList.NAME_KEY, "")?: ""
            val btCode = preferences.getString(idHeader + IOlyCameraEntryList.CODE_KEY, "")?: ""
    ...
    companion object
    {
        ...
        private const val BLE_SCAN_TIMEOUT_MILLIS = 8 * 1000 // 8秒間
